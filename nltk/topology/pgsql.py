    ...
    if cnx:
        ...
        for token in tokens:
            try:
                ...
                print("No wordforms found: '{}', {}!".format(token), e)
                import sys
                sys.exit(1)
...
#                 # pos3NT = un_pos3NT  # .filter_feature(SLOT_FEATURE, PERSONAL_FEATURE, INFLECTED_FEATURE)
#
#                 un_pos2NT = unify(pos2NT, unNT)
#                 # pos2NT = pos2NT  # .filter_feature(SLOT_FEATURE, PERSONAL_FEATURE, INFLECTED_FEATURE)
#                 unification_result.append((un_pos2NT, un_pos3NT))
#
#             if not unification_result:
#                 unification_result = ((pos2NT, pos3NT),)
#
#             for pos2NT, pos3NT in unification_result:
#
#                 if facultative:
#                     if pos2NT.has_feature({BRANCH_FEATURE: 'obligatory'}):
#                         pos2NT = pos2NT.filter_feature(BRANCH_FEATURE)
...
#         rhs_types = dict()
#         for rhs_item in rhs:
#             l = rhs_types.setdefault(rhs_item[TYPE], list())
#             l.append(rhs_item)
#
#         productions.update(open_disjunction(Production(nt, (word,))))
#         # productions.append(Production(nt, (word,)))
#         for alternative in itertools.product(*rhs_types.values()):
#             rhs_alternative = tuple(alternative)
#             productions.update(
#                 map(lambda production: production.process_inherited_features(), open_disjunction(Production(lhs, rhs_alternative))))
#
#     unificationCursor.close()
#     frameCursor.close()
#     if productions:
...
#             yield Production(lhs, rhs)
#     # else:
#     #     raise Exception("No productions found")
#     # return productions
